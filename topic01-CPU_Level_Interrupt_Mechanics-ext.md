...
### 1.4 分叉路标：中断、异常、系统调用在这里「分家」

这一章你会反复看到"异常""系统调用""trap"这些词——很自然会困惑：**它们不是中断，
为什么中断专题要碰它们？** 一句话答案：**因为它们和中断共享这一章讲的「最底层入口」，
但仅止于此——进了内核就各走各路。** 先把这张"分家图"立在这里，全章读起来就有方向。

**第一步：硬件用同一套管道把它们「陷入」内核（这一章的范围）**
- x86：异常和中断**共用同一张 IDT**（0–31 是异常，32–255 是中断向量），`idtentry`
  宏（§3.2）**同时**生成异常桩和中断桩。
- arm64：同步异常和异步 IRQ/FIQ **并排**在同一张 16 项向量表里（§4.1）。
- 两者最后都汇进**同一个通用入口层**（§5 的 `irqentry_*`）。

所以你**无法只讲中断入口而不碰到异常/syscall 入口**——它们是同一套桩、同一张表、
同一个通用层。这一章覆盖这段**共享入口**是必要的，不是跑题。

**第二步：进了内核立刻分家，本系列只跟「中断」这一支**

```
        硬件陷入（IDT 门 / arm64 向量表 / 通用入口层）── ★ 本章(Topic 01)讲这一段
                          │
        ┌─────────────────┼──────────────────────┐
     同步异常           异步中断                 半骑墙的特例
        │                 │                        │
  ┌─────┴──────┐          │                 ┌──────┴───────┐
缺页 do_page_fault  系统调用  generic_handle_irq  NMI(向量2)   #MC/SError
→ mm/ 子系统       syscall   → kernel/irq/      特殊入口      机器检查
（不在本系列）     分发表    ★本系列全部内容    (Topic 12)    （不在本系列）
                  （不在本系列）
```

**判别只靠一条轴：同步 vs 异步**——事件是「当前指令自己引起的」(同步=异常，如缺页、
syscall、断点 trap) 还是「外部、与当前指令无关」(异步=中断，如网卡、IPI)。
- **同步异常**（缺页 fault / 断点 trap / 除零）→ 走 `arch/*/mm/fault.c`、syscall 入口等
  **各自的子系统**，本系列**不讲它们的处理逻辑**。
- **异步中断**（IRQ/MSI/IPI）→ 走 `kernel/irq/`（genirq）→ **本系列从这里开始的全部内容**。
- **骑墙特例**：NMI 本质异步却占异常区向量、走特殊路径（Topic 12）；机器检查
  #MC/SError 是硬件异步错误却以异常形式送达——知道它们是"特例"即可。

**一眼验证**：`/proc/interrupts` 里**只有异步中断**，缺页和系统调用永远不出现——
因为它们根本不经过中断子系统、没有 `irq_desc`（topic00-ext 晦涩点 ②.5 详述了
"为什么 syscall/trap/异常不属于中断专题"）。

> **记住这张图**：本章是三条路的**共同起点**；从 §2 开始的具体内容，全部沿着
> "异步中断"那一条往下走。看到"异常""syscall"出现，知道那是在讲**共享入口**，
> 而不是本系列要展开的子系统。
