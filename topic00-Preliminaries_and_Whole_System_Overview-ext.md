...
|---|---|---|---|---|
| **进程/任务** | 能 | 能（GFP_KERNEL）| 系统调用、kthread、线程化 handler、**工作队列 worker** | `in_task()` |
| **软中断（softirq/tasklet）** | **不能** | 不能（只 GFP_ATOMIC）| 网络收包、定时器、tasklet | `in_serving_softirq()` |
| **硬中断** | **不能** | 不能 | top half、flow handler | `in_hardirq()` |
| **NMI** | **不能**（几乎啥都不能）| 不能 | 看门狗、perf | `in_nmi()` |

> **⚠️ "下半部 / bottom half" 是个被重载的词，别被它坑了（这是个高频困惑点）**
>
> 这张表第二行原本写"软中断/下半部"，容易让人误读成"**凡是下半部都不能睡**"——
> **错。** "下半部"有两个意思：
> - **狭义（指上下文）**：特指 **softirq 上下文**（softirq/tasklet 跑这里，原子、不能睡）。
>   内核的 `local_bh_disable`、`in_softirq()`、"BH" 都是这个狭义。表里第二行说的是它，**对**。
> - **广义（指"延迟工作"这件事）**：上半部推迟出去的**所有**后续工作，**包括工作队列、
>   线程化 IRQ**——而这些**能睡**。
>
> 关键：**工作队列不是一种独立的执行上下文**。它的 worker、线程化 IRQ 的 kthread，
> 都跑在**「进程/任务」那一行**（所以上表第一行特意补上了"工作队列 worker"）。于是：
>
> | 延迟（下半部）机制 | 跑在哪个上下文 | 能睡？ |
> |---|---|---|
> | softirq / tasklet | 软中断行 | ❌ 不能 |
> | **工作队列 / 线程化 IRQ** | **进程/任务行** | ✅ **能** |
>
> 所以精确的话是：**「softirq 上下文不能睡」永远对；「所有下半部不能睡」是错的**
> ——要睡眠就把活儿放进**工作队列或线程化 IRQ**（它们在进程上下文）。这个区分是
> Topic 07（线程化）和 Topic 09（工作队列 vs tasklet）的核心，那两章 ext 文档详述。
