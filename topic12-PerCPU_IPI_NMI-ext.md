...
> **路标：NMI 是"中断 vs 异常"分类里的骑墙特例**（呼应 topic00-ext 晦涩点 ②.5、
> topic01-ext §1.4 的分叉图）。回忆那条本质分类轴——同步（异常，当前指令引起）
> vs 异步（中断，外部引起）。**NMI 横跨两边、不服管**：
> - 它**本质是异步的**（来源在 CPU 外：看门狗硬件、性能计数器溢出、固件），按本质算"中断"；
> - 但在 x86 上它占的是**异常区的向量 2**（`X86_TRAP_NMI`，和缺页 #PF=14、断点 #BP=3
>   挤在 0–31 异常区里），走的也**不是**普通可屏蔽 IRQ 那条 `vector_irq[]`/`irq_desc` 路径，
>   而是**独立的 NMI 入口**（topic01-ext 提到的 IST 专用栈 + `register_nmi_handler`，见晦涩点 ⑦）；
> - arm64 上对应的异步硬件错误是 **SError**，同样以"异常"形式送达。
>
> 所以 NMI 既不像"普通设备中断"（不走 genirq 主路径），也不是"CPU 自产的同步异常"
> （它来自外部）。**它是那张分叉图里标着"半骑墙特例"的那一支。** 记住这个定位，
> 下面 NMI 的种种"特殊待遇"（不可屏蔽、专用栈、专用注册表、近乎啥都不能干）就有了来由
> ——它本就不在常规分类的任何一格里。
